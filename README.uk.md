...
дають підписи й типи; поле поза ними підписується своїм ключем у вигляді заголовка (`work_email` → `Work Email`), а `email` і `phone` розпізнаються за назвою. Формат
